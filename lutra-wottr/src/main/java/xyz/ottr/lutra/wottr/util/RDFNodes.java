...
public enum RDFNodes {
    ...
    public static String toString(PrefixMapping model, Collection<? extends RDFNode> nodes) {
        return nodes.stream()
            .map(node -> toString(model, node))
            .collect(Collectors.joining(", ", "[", "]"));
    }

    public static String toString(PrefixMapping model, RDFNode node) {
    ...
    public static String toString(PrefixMapping model, Node node) {
        return node.isVariable()
            ? node.toString()
            : toString(model, node.toString());
    }

    public static String toString(PrefixMapping model, String nodeString) {
        return (model == null)
            ? toString(nodeString)
